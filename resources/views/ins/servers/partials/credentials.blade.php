<form class="credential-form new-credential" style="background-color:#fafafa ">
    ...
    <div class="form-group">
        <button v-on:click="createCredential(server.user_id, server.id)" class="btn btn-primary">Save</button>
    </div>
</form>
<hr>

<template v-if="server.credentials.length > 0">
    <table class="table table-striped">
        <thead><tr><th>Name</th><th>Username</th><th>Password</th><th>Hostname</th><th>Port</th><th>Actions</th></tr></thead>
        <tbody>
            <tr v-for="credential in server.credentials">
