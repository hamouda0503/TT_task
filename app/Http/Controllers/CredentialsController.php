...
class CredentialsController extends BaseController {
	...
	// Get all credentials for the given server
	public function getServerCredentials($id){
		if( count(Credential::where('server_id',$id)->get()) === 0 ){
			if (!Input::get('password')) {
				return $this->setStatusCode(404)->makeResponse('No credentials found for this server');
			}
		}

		return $this->setStatusCode(200)->makeResponse('Found credentials for this server', Credential::where('server_id',$id)->get() );
	}
	...
	public function storeCredential(){
		...
		if (!Input::get('server_id')) {
			return $this->setStatusCode(406)->makeResponse('No server id is being passed');
		}
