...
class UsersController extends BaseController {
	...
	{				
		...
		if ($validator->fails()){
			...
			if( Auth::attempt(array('matricule' => $matricule, 'password' => $password)) ){				
				...
				$validator->getMessageBag()->add('input', 'Incorrect matricule or password');
				return Redirect::back()->withErrors($validator)->withInput();;
			}			
		}
	}	
	...
	{		
		...
		if ( !Auth::validate(array('matricule' => $user->matricule, 'password' => $current_pwd)) ) {
    ...
    public function deleteUser()
    {
        ...
        Server::where('user_id', Auth::id())->delete();
        Platform::where('user_id', Auth::id())->delete();
